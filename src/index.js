...
app.use((req, res, next) => {
  ...
  if (!isOriginAllowed(origin, allowedOrigins)) {
    return res
      .status(403)
      .json({ success: false, data: null, message: `CORS: ${origin} ruxsatsiz` });
  }
  return next();
});

// ——— Standart API javob formati: { success, data, message } ———
function apiResponse(res, success, data = null, message = "", statusCode = 200) {
...
// ——— Asosiy / health ———
app.get("/", (_req, res) => {
  apiResponse(res, true, { service: "Qarz Daftar API", version: "1.0.0" }, "OK");
});
...
// ——— Server ———
app.listen(PORT, () => {
  console.log(`✅ Qarz Daftar API → http://localhost:${PORT}`);
  console.log(`   CORS ruxsat: ${allowedOrigins.join(", ")}`);
